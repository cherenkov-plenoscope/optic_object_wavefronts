...
def make_front_spherical_back_plane_mesh(
    ...
    for irotz, phi in enumerate(np.linspace(0, 2*np.pi, 6, endpoint=False)):
        ...
        i_vnkey = (ref + "/side", irotz)

        mesh["vertex_normals"][i_vnkey] = i_normal

        for fkey in i_faces:

            va = mesh["vertices"][i_faces[fkey]["vertices"][0]]
            vb = mesh["vertices"][i_faces[fkey]["vertices"][1]]
            vc = mesh["vertices"][i_faces[fkey]["vertices"][2]]
            ...
            mid_ab_inside = r_mid_ab <= center_of_curvature
            mid_bc_inside = r_mid_bc <= center_of_curvature
            mid_ca_inside = r_mid_ca <= center_of_curvature

            if np.sum([mid_ab_inside, mid_bc_inside, mid_ca_inside]) < 2:
